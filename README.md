...
- **8 种声音风格**：从轻柔双音到醒目连响，时长约 1.1–2.8 秒，也支持导入 WAV、MP3、M4A、WMA。
- **最小化提醒**：开启兼容模式后，微信最小化时继续读取会话。
- **控制提醒频率**：连续消息间隔可设为 0–30 秒，默认 2 秒，避免一串消息连续响铃。
- **轻装运行**：原生 Windows 界面，主程序约 75 KB，解压即用，无需额外安装浏览器运行时。

当前版本：**v0.3.0**。适用于 **Windows 10 / 11 x64**，最小化兼容模式目前适配 **微信 4.1.13.63**。
...
想要明显一些的提醒，可以选择 **醒目连响**：三组双音，持续约 2.8 秒。喜欢日常提示感的，可以试试 **叮咚门铃** 或 **水晶三连**；也有轻柔双音、清脆铃音、轻敲木音、雷达呼叫和上行旋律可选。原有的三种音效也已延长并提高声音电平，已有联系人设置会继续保留。
...
**最小化兼容。** 微信当前版本默认没有提供完整的辅助访问控件树。用户开启兼容模式后，程序校验微信版本、DLL 的 SHA-256 和运行时指令，再启用微信进程中的一个辅助访问状态字节，让会话控件在最小化时仍可读取。监听期间定期复查；微信将状态重置为关闭值时，重新校验后自动恢复，无需手动反复开关。关闭兼容模式或正常退出时还原由轻响做出的修改。这是一项针对特定微信版本的非官方适配，实现与验证记录见 [兼容性说明](docs/compatibility.md)。
